...
import { carritoActual } from "../shop/Products";
interface Producto {
  id: number;
  nombre: string;
  descripcion: string;
  precio: number;
  url_imagen: string;
  qty: number; // <--- Agregamos esto aquí
}
const Cart: React.FC = () => {
  ...
  useEffect(() => {
    // Intentamos cargar primero del localStorage por seguridad, si no, usamos tu array global
    const cacheado = localStorage.getItem("carrito");

    if (cacheado) {
      setItems(JSON.parse(cacheado));
    } else if (carritoActual && carritoActual.length > 0) {
      // Como en tu función anterior agregabas el producto completo, nos aseguramos de mapear las cantidades correctamente
      const unificados = carritoActual.reduce((acc: Producto[], prod: any) => {
        const existe = acc.find((item) => item.id === prod.id);
        if (existe) {
          existe.qty += 1;
        } else {
      ...
      setItems(unificados);
      localStorage.setItem("carrito_salon", JSON.stringify(unificados));
    }
  }, []);

  useEffect(() => {
    if (items.length > 0) {
      localStorage.setItem("carrito_salon", JSON.stringify(items));
    }
  }, [items]);
  ...
  const removeItem = (id: number) => {
    ...
    localStorage.setItem("carrito", JSON.stringify(nuevosItems)); // Actualiza inmediato al borrar
  };
              ...
              <div className="products-list">
                {items.map((p) => (
                  <div className="product-item" key={p.id}>
                    <div className="product-img">{p.url_imagen}</div>
                    <div className="product-info">
                      <div className="product-name">{p.nombre}</div>
                      <div className="product-desc">{p.precio}</div>
            ...
            <div className="checkout-layout">
              <div className="order-summary-card">
                {items.map((p) => (
                  <div className="order-item" key={p.id}>
                    <div className="order-img">{p.url_imagen}</div>
                    <div className="order-item-info">
                      <div className="order-item-name">{p.nombre}</div>
                      <div className="order-item-desc">{p.descripcion}</div>
              ...
              <div className="yape-card">
                ...
                <div className="info-note">
                  📋 Por favor, una vez realizado el pago, envía la captura de
            ...
            <div className="checkout-layout">
              <div className="order-summary-card">
                ...
                <div className="editorial-quote">
                  ✦ "Cada detalle en el Atelier está diseñado para elevar tu
